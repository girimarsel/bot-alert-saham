...
# Python 3.11+. Tanpa library eksternal.

import os, json, re, hashlib, urllib.request, urllib.parse, ssl
from datetime import datetime, timezone
from email.utils import parsedate_to_datetime
from xml.etree import ElementTree as ET

# ========= ENV (HARUS diisi via GitHub Secrets) =========
TELEGRAM_BOT_TOKEN = os.environ.get("TELEGRAM_BOT_TOKEN", "")
TELEGRAM_CHAT_ID   = os.environ.get("TELEGRAM_CHAT_ID", "")
STATE_FILE = "sent_items.json"
MAX_MSG_PER_CYCLE = int(os.environ.get("MAX_MSG_PER_CYCLE", "12"))

# ========= Google News RSS (stabil) =========
NEWS_SITES = [
    "emitennews.com","kontan.co.id","bisnis.com","cnbcindonesia.com",
    "antaranews.com","investor.id","idxchannel.com","kompas.com","detik.com"
]
NEWS_QUERIES = [
...
def build_feeds():
    feeds=[]
    site_filter=" OR ".join([f"site:{d}" for d in NEWS_SITES])
    for q in NEWS_QUERIES:
        full=f"({q}) ({site_filter})"
        url="https://news.google.com/rss/search?"+urllib.parse.urlencode(
            {"q":full,"hl":"id","gl":"ID","ceid":"ID:id"}
        )
        feeds.append(url)
    return feeds
FEEDS = build_feeds()
...
KEYMAP = {
    ...
    "Komoditas / Makro": ["batubara","coal","hba","cpo","minyak","oil","nikel","timah","emas","tembaga"],
    "Ekspansi / Proyek": ["ekspansi","pabrik","kapasitas","investasi","joint venture","kerja sama","kemitraan","perluasan","proyek baru"],
}
IMPACT = {
...
def http_get(url, timeout=20):
    req=urllib.request.Request(url, headers={"User-Agent":UA,"Referer":"https://news.google.com/"})
...
def parse_rss(xml_bytes):
    ...
    if root.tag.lower().endswith("feed"):
        ...
        for e in root.findall(".//a:entry" if ns else ".//entry", ns):
            ...
            summary=(e.findtext("a:summary" if ns else "summary") or e.findtext("a:content" if ns else "content") or "")
            date_txt=(e.findtext("a:updated" if ns else "updated") or e.findtext("a:published" if ns else "published") or "")
...
def as_jakarta(dt):
    ...
    except: return dt if dt.tzinfo else dt.replace(tzinfo=timezone.utc)
def domain_of(url):
    try: return urllib.parse.urlsplit(url).netloc.replace("www.","")
    except: return "sumber"

# Kirim → return True/False
...
def format_msg(cat,title,url,pub):
    hint=IMPACT.get(cat, IMPACT["Lainnya"])
    tgl=as_jakarta(pub).strftime("%d %b %Y %H:%M WIB")
    src=domain_of(url)
    emoji={"Akuisisi / MTO / Merger":"📣","Rights Issue / Private Placement":"🧾",
           "IPO / Delisting / Stock Action":"🆕","Buyback":"🔁","Dividen":"💰",
           "Laporan Keuangan":"📊","Regulasi / Kebijakan":"⚖️",
           "Komoditas / Makro":"🌐","Ekspansi / Proyek":"🏗️","Lainnya":"📢"}.get(cat,"📢")
    return (f"{emoji} <b>ALERT SAHAM – {cat}</b>\n\n"
            f"📰 <b>Judul:</b> {title}\n"
            f"📅 <b>Tanggal:</b> {tgl}\n"
            f"🗞️ <b>Sumber:</b> {src}\n"
...
def run_once():
    ...
    for pub,cat,title,link,key in found[:MAX_MSG_PER_CYCLE]:
        ok=send_telegram(format_msg(cat,title,link,pub))
        if ok:
